...
class AdvancedUsageCase1 : public ShaderStorageBufferObjectBase
{
	...
	virtual long Run()
	{
		...
		/* transform buffer */
		{
			mat4 data[] = { transpose(Translation(-0.5f, -0.5f, 0.0f)), transpose(Translation(0.5f, -0.5f, 0.0f)),
							transpose(Translation(-0.5f, 0.5f, 0.0f)), transpose(Translation(0.5f, 0.5f, 0.0f)) };
			glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, m_storage_buffer[0]);
			glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(data), data, GL_STATIC_DRAW);
		}
		/* material buffer */
...
ShaderStorageBufferObjectTests::ShaderStorageBufferObjectTests(glcts::Context& context)
