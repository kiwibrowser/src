...
// chromecast/browser/tts/extension_api.
...
const std::vector<extensions::TtsVoice>* GetVoicesInternal(
  ...
  const std::vector<extensions::TtsVoice>* voices =
      TtsEngineExtensionObserver::GetInstance(context)->GetRuntimeVoices(
          extension->id());
...
void TtsExtensionEngine::GetVoices(content::BrowserContext* browser_context,
  ...
  for (iter = extensions.begin(); iter != extensions.end(); ++iter) {
    ...
    const std::vector<extensions::TtsVoice>* tts_voices =
        GetVoicesInternal(browser_context, extension);
    if (!tts_voices)
      continue;
...
void TtsExtensionEngine::Speak(Utterance* utterance, const VoiceData& voice) {
  ...
  auto event = std::make_unique<extensions::Event>(
      ...
      std::move(args), utterance->browser_context());
  EventRouter::Get(utterance->browser_context())
...
void TtsExtensionEngine::Pause(Utterance* utterance) {
  ...
  auto event = std::make_unique<extensions::Event>(
      ...
      std::move(args), utterance->browser_context());
  EventRouter* event_router = EventRouter::Get(utterance->browser_context());
  std::string id = utterance->extension_id();
  event_router->DispatchEventToExtension(id, std::move(event));
  WarnIfMissingPauseOrResumeListener(utterance->browser_context(), event_router,
                                     id);
}
...
bool TtsExtensionEngine::LoadBuiltInTtsExtension(
  ...
  if (TtsEngineExtensionObserver::GetInstance(browser_context)
          ->SawExtensionLoad(extension_misc::kSpeechSynthesisExtensionId,
                             true)) {
    return false;
  }
...
ExtensionTtsEngineUpdateVoicesFunction::Run() {
  ...
  TtsEngineExtensionObserver::GetInstance(browser_context())
      ->SetRuntimeVoices(std::move(tts_voices), extension()->id());
...
ExtensionTtsEngineSendTtsEventFunction::Run() {
  ...
  if (event_type == constants::kEventTypeStart) {
    controller->OnTtsEvent(utterance_id, TTS_EVENT_START, char_index,
                           std::string());
  } else if (event_type == constants::kEventTypeEnd) {
    controller->OnTtsEvent(utterance_id, TTS_EVENT_END, char_index,
                           std::string());
  } else if (event_type == constants::kEventTypeWord) {
    controller->OnTtsEvent(utterance_id, TTS_EVENT_WORD, char_index,
                           std::string());
  } else if (event_type == constants::kEventTypeSentence) {
    controller->OnTtsEvent(utterance_id, TTS_EVENT_SENTENCE, char_index,
                           std::string());
  } else if (event_type == constants::kEventTypeMarker) {
    controller->OnTtsEvent(utterance_id, TTS_EVENT_MARKER, char_index,
                           std::string());
  } else if (event_type == constants::kEventTypeError) {
    std::string error_message;
    event->GetString(constants::kErrorMessageKey, &error_message);
    controller->OnTtsEvent(utterance_id, TTS_EVENT_ERROR, char_index,
                           error_message);
  } else if (event_type == constants::kEventTypePause) {
    controller->OnTtsEvent(utterance_id, TTS_EVENT_PAUSE, char_index,
                           std::string());
  } else if (event_type == constants::kEventTypeResume) {
    controller->OnTtsEvent(utterance_id, TTS_EVENT_RESUME, char_index,
                           std::string());
  } else {
